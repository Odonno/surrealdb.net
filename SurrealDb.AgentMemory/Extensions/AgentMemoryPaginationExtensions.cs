...
public static class AgentMemoryPaginationExtensions
{
    ...
    /// Terminates on the token (never on a short page) and fails loudly if the server repeats a cursor instead of advancing.
    /// </summary>
    ...
    )
    {
        ...
        while (true)
        {
            ...
            // The token is the only reliable end-of-walk signal: absent (or empty) on the last page.
            // A page bounded in the database and then filtered for visibility can return fewer rows than `limit` while more pages remain.
            if (string.IsNullOrEmpty(next))
            {
                yield break;
            }

            // A cursor that repeats means the server is not advancing; following it would spin forever.
            // Fail loudly rather than hang or truncate.
            if (!seen.Add(next))
            {
    ...
    /// This is an unbounded read by construction;
    /// prefer page-at-a-time <see cref="WalkPagesAsync"/> for anything user-facing and large.
    ...
    )
    {
        ...
        await foreach (
            var page in api.WalkPagesAsync(fetchPage, pageOf, cancellationToken)
