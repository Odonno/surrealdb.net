...
internal sealed class ToIntermediateExpressionVisitor : ExpressionVisitor
{
    ...
    )
    {
        ...
        _sourceExpressionParameters.Add(keySelector.Parameters[0], sourceSelect.Projection);
        ...
        var innerSubqueryExpression = sourceSelect.AppendWhere(
            Expression.Equal(keyExpression, keyExpression) // TODO : Bind one to the "parent" select(or)
        );
    ...
    private CustomExpression BindCount(
        Type resultType,
        Expression source,
        LambdaExpression? predicate
    )
    {
        ...
        var innerQuery = sourceSelect
            .WithProjection(new CountFieldProjectionExpression(typeof(CountProjector), predicate))
            .WithGroupAll();

        // 💡 We simulate the type so it can be interpreted later as an "Array"
        return CreateTopLevelExpression(
            resultType,
            typeof(CountProjector),
            nameof(CountProjector.count),
            innerQuery,
            typeof(CountProjector[])
        );
    }
    ...
    private IntermediateExpression BindDistinct(Type resultType, Expression source)
    {
        var sourceSelect = (SelectExpression)Visit(source);
