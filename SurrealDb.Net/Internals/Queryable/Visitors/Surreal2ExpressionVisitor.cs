...
internal sealed class Surreal2ExpressionVisitor : ExpressionVisitor
{
    ...
    public override Expression? Visit(Expression? node)
    {
        ...
        var x = _sourceExpressionParameters.FirstOrDefault((kv) => kv.Value == node);
        if (x.Key is not null)
        {
            _parametersNestedLevels.TryAdd(x.Key, _currentNestedSelectLevel);
        }
    ...
    private SelectStatementExpression BindSelect(SelectExpression selectExpression)
    {
        ...
        return new SelectStatementExpression(fields, what, cond, grouping, ordering, limit, start);
    }
    ...
    private WhatExpression BindSource(SourceExpression sourceExpression)
    {
        ...
        {
            Expressions.Intermediate.TableSourceExpression tableSourceExpression =>
                (TableValueExpression)Visit(tableSourceExpression.Table)!,
            Expressions.Intermediate.SelectSourceExpression selectSourceExpression =>
                new SubqueryValueExpression(
    ...
    private GroupingExpression BindGroup(GroupsExpression groupsExpression)
    {
        ...
        var idiomExpression = Visit(groupsExpression.Expression)?.ToIdiom();
    ...
    private Expression BindEnumerableMethodCall(MethodCallExpression node)
    {
        ...
        if (
            node.Method.Name.Equals(nameof(Enumerable.Max), StringComparison.Ordinal)
            && node.Arguments.Count == 1
        )
        {
            var valueExpression = Visit(node.Arguments[0])?.ToValue();
            ...
            // TODO : second arg?
            return new FunctionValueExpression("array::max", [valueExpression]);
        }
        if (
            node.Method.Name.Equals(nameof(Enumerable.Min), StringComparison.Ordinal)
            && node.Arguments.Count == 1
        )
        {
            var valueExpression = Visit(node.Arguments[0])?.ToValue();
            ...
            // TODO : second arg?
            return new FunctionValueExpression("array::min", [valueExpression]);
        }
#if NET7_0_OR_GREATER
        if (
